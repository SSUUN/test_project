...
def date_selecter(ns,samp,seed):
    ...
    for j in range(6):
        ...
        nx=np.load(f"ppp/open/datas/data_31/x_{j}.npy")
        ny=np.load(f"ppp/open/datas/data_31/y_{j}.npy")
        lens = nx.shape[0]
        ...
        for i in range(5):
            ...
            idx1=np.random.choice(idx,size=int(len(idx)*(ns[i]*0.9)),replace=False,)
            # time is fucking over so this test_train_split 
            lks=int(len(idx1)*(ns[i]*0.1))
                ...
                for n1s in [1]:
                    for n0s in [1]:
                        ...
                        try:
                            ...
                            for kima in [6]:
                                for subcol in [sudco]:
                                    ...
                                    xgb_model = xgb.train(
                                        ...
                                        callbacks=[xgb.callback.TrainingCheckPoint(f'ppp/open/models3_5_5_all_{fkn2}', interval=2000)],
                                        verbose_eval=30
                                    )
                                    ...
                                    xgb_model.save_model(f"ppp/open/models3_5_5_all_{fkn2}/xgb_an_{fkn2}_{fkn}_{25}_{pq1}_{kima}_{str(subcol).replace('.','')}_{str(flflfl).replace('.','')[:4]}.model")
                                    # 모델 저장
                            ...
                            fkn2+=100
                            del y_pred,flflfl,x_tr, y_tr, x_te, y_te
                            time.sleep(2)
